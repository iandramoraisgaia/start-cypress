...
## 01/09/2026 — Primeiro teste real na TestIO (app de e-commerce/streaming, iOS)

- O que fiz: Participei no meu primeiro teste real de crowd-testing na TestIO, para uma app iOS de um cliente da plataforma. Testei duas funcionalidades: App startup (arranque da app após instalação nova) e Login (campos, validação, recuperação de password, redirecionamento após autenticação). Tentei também uma reprodução de bug já reportado por outro tester.
- Dificuldade: Ao testar "App startup" com instalação nova, encontrei sozinha um comportamento estranho (falta de ecrã de boas-vindas e pedido de notificações), mas ao investigar percebi que já era um bug conhecido, reportado por outro tester. Também descobri que apagar e reinstalar a app não simula uma instalação verdadeiramente nova num dispositivo já usado, porque o iOS guarda credenciais no Keychain, independentes da desinstalação da app. Por fim, ao tentar anexar um vídeo à submissão de um bug, recebi erro de formato/extensão.
- Como resolvi: Confirmei a suspeita do bug já conhecido comparando a minha observação com a lista de "Known bugs" antes de submeter, evitando um duplicado. Sobre o Keychain, percebi a limitação e passei a documentar essa variação nas minhas notas em vez de a tratar como resultado inconsistente. Sobre o anexo, pesquisei os requisitos oficiais da TestIO (screencasts têm de ser MP4, até 25MB, até 60s para Bug Reports) e identifiquei que o meu vídeo estava em .mov/HEVC e acima do limite de tamanho — a correção é mudar a definição da câmara do iPhone para "Mais Compatível" antes de gravar, e manter os vídeos curtos.
- O que aprendi: A importância de verificar sempre a lista de bugs conhecidos antes de submeter. A limitação real de testar "fresh install" num dispositivo pessoal já usado. Os requisitos técnicos exatos de anexos da TestIO. A prática de gravar ecrã desde o início do teste. E, criticamente: **testers da TestIO têm obrigação contratual de confidencialidade — nunca mencionar publicamente o nome de clientes, apps específicas ou detalhes de bugs fora da plataforma** (LinkedIn, GitHub, portefólio incluído).
- Evidência: sessões de teste completas em App startup e Login na TestIO, sem bugs novos submetidos mas com cobertura documentada; tentativa de reprodução de bug conhecido.